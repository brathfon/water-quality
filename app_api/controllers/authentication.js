...
module.exports.login = function (req, res) {
  ...
  query = 'call login(';
  query += db.pool.escape(workerEmail) + ")";  // can't be blank
  ...
  //debugLogin(chalk.green("fields : " + util.inspect(fields, false, null)));
    ...
    if (err) {
      helpers.sendJsonSQLErrorResponse("Error retrieving data from database for user " + workerEmail + " role information",
                               "Danger",
                               err,
                               data,
                               res);
...
module.exports.setPassword = function (req, res) {
  ...
  if (!req.body.worker_id ) { helpers.sendJsonResponse(res, 400, {"message": "login(): worker_id param not passed"}); return;};
  if (!req.body.password )  { helpers.sendJsonResponse(res, 400, {"message": "login(): password param not passed"}); return; };
  ...
  //debugPassword(chalk.green("fields : " + util.inspect(fields, false, null)));
    ...
    if (err) {
      helpers.sendJsonSQLErrorResponse("Error retrieving data from database for user " + workerID + " role information",
                                       "Danger",
                                       err,
                                       data,
                                       res);
      ...
      if (rows[0].length === 0) {
        //data['token'] = "NO_MATCHING_EMAIL_FOUND";
        helpers.sendJsonSQLErrorResponse("Setting password for " + workerID + " did not return any role information.",
                                         "Danger",
                                          err,
                                          data,
                                          res);
