...
// This is to get away from having a bunch of files with just the path in them.  Pass the pass to the files in as an arg.
// Use this from now on (9/7/2020)
...
const printUsage = function () {
  console.log(`Usage: ${scriptname} <current-spread-sheet-dir>`);
}
...
let gDriveDownloadsPath  = process.argv[2];
...
var readSpreadSheetFiles = function (data, endConnection, callback) {
  ...
  //data['sessions'] = rss.readTeamSheets(process.env.WQ_FILES + "/gdrive-downloads/data-sessions-1-75w-1-50s");
  data['sessions'] = rss.readTeamSheets(gDriveDownloadsPath);
  //data['sessions'] = rss.readTeamSheets("/Users/bill/development/water-quality/legacy-to-gdrive/scripts/legacy-to-gdrive-conversion/tab-separated-files-after");
...
var createSqlStatements = function (data, callback) {
  ...
  for (lab_session_code in data.sessions) {
    ...
    //console.log("samples " + util.inspect(data.sessions[lab_session_code], false, null));
    ...
    if (! lab_id) {
       console.log(`ERROR: lab_id lookup failed for "${lab}"`);
       console.error(`ERROR: lab_id lookup failed for "${lab}"`);
       process.exit(1);
    };
    ...
    */
    ...
    for (j = 0; j < samples.length; ++j) {
      ...
      if (! site_id) {
        console.log(`ERROR: site_id lookup failed for "${hui_abv}"`);
        console.error(`ERROR: site_id lookup failed for "${hui_abv}"`);
        process.exit(1) };
