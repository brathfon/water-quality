      ...
      loginInfoSubmitted : function() {
        ...
        .catch( (error) => {
          if (error.response) {
           ...
           errorMsg = '';
           // is error is known, generated by API. 500 status is SQL and
           // could have multiple errors, but usually only one.
           if ((error.response.status === 500) && (error.response.data.errors)) {
              this.loginErrors = this.loginErrors.concat(error.response.data.errors);
           }
